        ...
        >
          <div class="bg-white rounded-xl shadow-2xl border border-slate-200 w-[640px] max-w-[95vw] flex flex-col overflow-hidden max-h-[88vh]">
            ...
            <div class="px-5 py-4 space-y-4 overflow-y-auto max-h-[60vh]">
              ...
              <div v-if="detailsService.name === 'PostgreSQL' || detailsService.database">
                ...
                <div class="grid grid-cols-3 gap-2 text-xs mb-2">
                  <!-- Default Database -->
                  <div class="bg-slate-50 p-2.5 rounded-lg border border-slate-200/80">
                    <div class="text-[10px] font-medium text-slate-400 uppercase">Default Database</div>
                    <div class="font-mono font-bold text-slate-800 text-xs mt-0.5 truncate flex items-center justify-between">
                      <span>{{ detailsService.database || 'deloc_db' }}</span>
                      <button
                        @click="copyEndpoint(detailsService.database || 'deloc_db', 'db-copy')"
                        class="text-slate-400 hover:text-slate-600 p-0.5 cursor-pointer"
                        title="Copy database name"
                      >
                        <Check v-if="copiedKey === 'db-copy'" class="w-2.5 h-2.5 text-emerald-600" />
                        <Copy v-else class="w-2.5 h-2.5" />
                      </button>
                    </div>
                  </div>
                  ...
                  <!-- Username -->
                  <div class="bg-slate-50 p-2.5 rounded-lg border border-slate-200/80">
                    <div class="text-[10px] font-medium text-slate-400 uppercase">Username</div>
                    <div class="font-mono font-bold text-slate-800 text-xs mt-0.5 truncate flex items-center justify-between">
                      <span>{{ detailsService.user || 'postgres' }}</span>
                      <button
                        @click="copyEndpoint(detailsService.user || 'postgres', 'user-copy')"
                        class="text-slate-400 hover:text-slate-600 p-0.5 cursor-pointer"
                        title="Copy username"
                      >
                        <Check v-if="copiedKey === 'user-copy'" class="w-2.5 h-2.5 text-emerald-600" />
                        <Copy v-else class="w-2.5 h-2.5" />
                      </button>
                    </div>
                  </div>
                  ...
                  <!-- Password -->
                  <div class="bg-slate-50 p-2.5 rounded-lg border border-slate-200/80">
                    <div class="text-[10px] font-medium text-slate-400 uppercase">Password</div>
                    <div class="font-mono font-bold text-slate-800 text-xs mt-0.5 truncate flex items-center justify-between">
                      <span>{{ showPassword ? (detailsService.password || 'postgres') : '••••••••' }}</span>
                      <div class="flex items-center gap-1">
                        ...
                        >
                          <EyeOff v-if="showPassword" class="w-2.5 h-2.5" />
                          <Eye v-else class="w-2.5 h-2.5" />
                        </button>
                        <button
                          @click="copyEndpoint(detailsService.password || 'postgres', 'pw-copy')"
                          class="text-slate-400 hover:text-slate-600 p-0.5 cursor-pointer"
                          title="Copy password"
                        >
                          <Check v-if="copiedKey === 'pw-copy'" class="w-2.5 h-2.5 text-emerald-600" />
                          <Copy v-else class="w-2.5 h-2.5" />
                        </button>
                      </div>
                    </div>
                ...
                <!-- Connection String -->
                <div class="p-2 rounded bg-slate-50 border border-slate-200/80 font-mono text-[11px] text-slate-700 flex items-center justify-between">
                  <span class="truncate">
                    postgresql://{{ detailsService.user || 'postgres' }}:{{ detailsService.password || 'postgres' }}@localhost:{{ detailsService.endpoints?.[0]?.port || 5432 }}/{{ detailsService.database || 'deloc_db' }}
                  </span>
                  <button
                    @click="copyEndpoint('postgresql://' + (detailsService.user || 'postgres') + ':' + (detailsService.password || 'postgres') + '@localhost:' + (detailsService.endpoints?.[0]?.port || 5432) + '/' + (detailsService.database || 'deloc_db'), 'conn-copy')"
                    class="px-2 py-0.5 text-[10px] bg-white hover:bg-slate-100 text-slate-600 rounded border border-slate-200 flex items-center gap-1 cursor-pointer shrink-0 ml-2"
                    title="Copy connection string"
                  >
                    <Check v-if="copiedKey === 'conn-copy'" class="w-2.5 h-2.5 text-emerald-600" />
                    <Copy v-else class="w-2.5 h-2.5" />
                    <span>{{ copiedKey === 'conn-copy' ? 'Copied' : 'Copy URI' }}</span>
                  </button>
                </div>
              </div>
...
function openDetails(service) {
  ...
  if (!detailsService.value.database) detailsService.value.database = 'deloc_db'
  if (!detailsService.value.user) detailsService.value.user = 'postgres'
  if (!detailsService.value.password) detailsService.value.password = 'postgres'
